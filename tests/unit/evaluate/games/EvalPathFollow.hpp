...
TEST_CASE("EvalPathFollow_PathFollowEvaluator", "[evaluate/games]"){
  { // LoadMap
    ...
    evaluator.LoadMap("evaluate/games/path_follow_files/test_map_straight.txt");
    ...
    { // Check each individual tile, but hide in a fold for readability in most editors
      ...
      CHECK(evaluator.path_data_vec[0].grid.GetSymbol(2,0) == 'O');
    ...
    evaluator.LoadMap("evaluate/games/path_follow_files/test_map_turns.txt");
    ...
    { // Check each individual tile, but hide in a fold for readability in most editors
      ...
      CHECK(evaluator.path_data_vec[1].grid.GetSymbol(1,1) == 'O');
  ...
  { // LoadAllMaps
    emp::Random rand(200);
    mabe::PathFollowEvaluator evaluator = mabe::PathFollowEvaluator(rand);
    evaluator.LoadAllMaps("evaluate/games/path_follow_files/test_map_straight.txt;evaluate/games/path_follow_files/test_map_turns.txt");
    CHECK(evaluator.GetNumMaps() == 2); 
    CHECK(evaluator.path_data_vec.size() == 2);
    CHECK(evaluator.path_data_vec[0].grid.GetWidth() == 5);  // Num cols
    CHECK(evaluator.path_data_vec[0].grid.GetHeight() == 5); // Num rows
  ...
  { // Initialize to first map without randomizing cues
    ...
    CHECK(state.forward_cue == 0);
    CHECK(state.left_cue == 1);
  ...
  { // Initialize to first map and randomize cues
    ...
    CHECK(evaluator.GetNormalizedScore(state) == 0);
    CHECK(state.forward_cue == 0);
    CHECK(state.right_cue == 653304);
    CHECK(state.left_cue == 841936);
    CHECK(state.empty_cue == -1);
  ...
  { // Initialize state to the second map 
    ...
    evaluator.LoadAllMaps(
        "evaluate/games/path_follow_files/test_map_turns.txt;evaluate/games/path_follow_files/test_map_straight.txt");
    evaluator.randomize_cues = false;
    mabe::PathFollowState state;
    state.cur_map_idx = 1;
    evaluator.InitializeState(state, false);
  ...
  { // RotateRight
    ...
    CHECK(state.status.GetX() == 2);
    CHECK(state.status.GetY() == 0);
    CHECK(state.status.GetFacing() == 5); // South
    evaluator.RotateRight(state);
    CHECK(state.status.GetX() == 2);
    CHECK(state.status.GetY() == 0);
    CHECK(state.status.GetFacing() == 6); // Southwest
    evaluator.RotateRight(state);
    CHECK(state.status.GetFacing() == 7); // West
    evaluator.RotateRight(state);
    CHECK(state.status.GetFacing() == 0); // Northwest
    evaluator.RotateRight(state);
    CHECK(state.status.GetFacing() == 1); // North
  }
  { // RotateLeft
    emp::assert_clear();
    emp::Random rand(900);
    ...
    CHECK(state.status.GetX() == 2);
    CHECK(state.status.GetY() == 0);
    CHECK(state.status.GetFacing() == 4); // Southeast
    evaluator.RotateLeft(state);
    CHECK(state.status.GetFacing() == 3); // East
    evaluator.RotateLeft(state);
    CHECK(state.status.GetFacing() == 2); // Northeast
    evaluator.RotateLeft(state);
    CHECK(state.status.GetFacing() == 1); // North
  }
  { // Sense
    ...
    { // Random cues
      ...
      CHECK(state.empty_cue == -1);
      CHECK(state.forward_cue == 0);
      CHECK(state.left_cue == 264083);
      CHECK(state.right_cue == 943441);
      CHECK(state.status.GetX() == 1);
      CHECK(state.status.GetY() == 1);
      CHECK(evaluator.Sense(state) == state.forward_cue); // Start tile gives forward cue
      state.status.SetPos(2,1);
