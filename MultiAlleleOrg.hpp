...
namespace mabe {
  ...
  public:
    ...
    size_t Mutate(emp::Random & random) override {
     const size_t num_muts = SharedData().mut_dist.PickRandom(random);
      ...
      if (num_muts == 1) {
        ...
        size_t pos1 = random.GetUInt(num_alleles.size());
        while(pos1 == alleles[pos]){
            pos1 = random.GetUInt(num_alleles.size());
        }
        alleles[pos] = pos1;
        return 1;
      }
    ...
    void Randomize(emp::Random & random) override {
      //emp::RandomizeBitVector(bits, random, 0.5);  // old one:  probability is 1/num_alleles
      emp::RandomizeVector(alleles, random, 0, alleles.size());  //Go to Definition of RandomizeVector
    }

    void Initialize(emp::Random & random) override {
      if (SharedData().init_random) emp::RandomizeVector(alleles, random, 0, alleles.size()); //old one was there
    }

    /// Put the bits in the correct output position.
    void GenerateOutput() override {
      SetTrait<genome_t>(SharedData().output_name, alleles);   // emp::BitVector change to genome_t
    }

    /// Setup this organism type to be able to load from config.
    void SetupConfig() override {
      GetManager().LinkFuns<size_t>([this](){ return alleles.size(); },
                       [this](const size_t & N){ return alleles.Resize(N); },
      ...
      GetManager().LinkVar(SharedData().num_alleles, "num_alleles",
                      "Number of alleles in our genome");
    }
    ...
    void SetupModule() override {
      ...
      GetManager().AddSharedTrait(SharedData().output_name,
                                  ...
                                  emp::BitVector(0)); // emp::BitVector(0) to emp::vector<size_t> genome_t?
    }
  };

  MABE_REGISTER_ORG_TYPE(MultiAlleleOrg, "Organism consisting of a series of N bits.");
}
