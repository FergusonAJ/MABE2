...
namespace mabe {
  ...
  public:
    OneStepMutants(mabe::MABE & control,
      ...
      : Module(control, name, desc), source_pop_id(_source_pop_id), mutant_pop_id(_mutant_pop_id),
        update_step(_update_step),
        min_mutation_idx(_min_mutation_idx), max_mutation_idx(_max_mutation_idx)
    {
    }
    ...
    void OnUpdate(size_t update) override {
      if(update % update_step == 0){
        ...
        const Organism & org = source_pop[0];
