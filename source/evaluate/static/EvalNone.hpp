/**
 *  @note This file is part of MABE, https://github.com/mercere99/MABE2
 *  @copyright Copyright (C) Michigan State University, MIT Software license; see doc/LICENSE.md
 *  @date 2019-2021.
 *
 *  @file  EvalNone.hpp
 *  @brief MABE Evaluation module for NK Landscapes
 */

#ifndef MABE_EVAL_NONE_H
#define MABE_EVAL_NONE_H

#include "../../core/MABE.hpp"
#include "../../core/Module.hpp"

namespace mabe {

  class EvalNone : public Module {
  private:

  public:
    EvalNone(mabe::MABE & control,
           const std::string & name="EvalNone",
           const std::string & desc="Module that can call GenerateOutput on orgs")
      : Module(control, name, desc)
    {
      SetEvaluateMod(true);
    }
    ~EvalNone() { }

    // Setup member functions associated with this class.
    static void InitType(emplode::TypeInfo & info) {
      info.AddMemberFunction("EVAL",
                             [](EvalNone & mod, Collection list) { return mod.Evaluate(list); },
                             "Calls GenerateOutput on all orgs");
    }

    void SetupConfig() override {
    }

    void SetupModule() override {
    }

    double Evaluate(const Collection & orgs) {
      // Loop through the population and evaluate each organism.
      mabe::Collection alive_orgs( orgs.GetAlive() );
      for (Organism & org : alive_orgs) {
        org.GenerateOutput();
      }
      return 0;
    }

    // If a population is provided to Evaluate, first convert it to a Collection.
    double Evaluate(Population & pop) { return Evaluate( Collection(pop) ); }

    // If a string is provided to Evaluate, convert it to a Collection.
    double Evaluate(const std::string & in) { return Evaluate( control.ToCollection(in) ); }
  };

  MABE_REGISTER_MODULE(EvalNone, "Simply calls GenerateOutput on orgs");
}

#endif
